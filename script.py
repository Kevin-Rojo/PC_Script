...
# single email
if option1 == '1':
    #En mi caso utilizamos la opcion 1 para evotar problemas por el spam... aqui se selecciona la direccion de correo que se va a atacar
    to = input(setprompt(["1"], "Send email to"))

# mass emailer 
if option1 == '2':
    #esta opcion se diferencia ya que se agrega una ruta con un archivo con una lista de email
    print ("""
 ...
 #pise el archivo una ves si no manda un mensaje de error y lo vuelve a pedir hasta que se agregue bien
    filepath = input(
...
#aqui se selecciona si usaras una cuenta predeterminada la cual en mi caso es gmail u otro servidor personal puede ser hotmail, yahoo. 
if option1 != "99":
    #utilice la opcion de your own server el cual fue hotmail
    ...
    if relay == '1':
        # aqui se valida si la direccion contiene uno de los proveedores de email para asignar el servidor smtp y el puerto
        provideruser = input(
            setprompt(["1"], "Your %s email address" % (email_provider)))
        from_address = provideruser
        # se agrega un nombre que ira en la cabecera del correo
        from_displayname = input(
            setprompt(["1"], "The FROM NAME the user will see"))
        #aqui se ingresa a contraseña del correo
    ...
    # se especifica la prioridad con la que aparecera el correo, aunque se selecciones high priority puede aparecer en correo no deseado
    ...
    #pregunta si se quiere agregar un archivo dentro del correo pero en este caso lo dejamos vacio
    file_format = ""
    yesno = raw_input("Do you want to attach a file - [y/n]: ")
    if yesno.lower() == "y" or yesno.lower() == "yes":
        #si se selecciona "y" entonces pide la ruta del archivo
    ...
    inline_files = []
    #no se muy bien a que se refiera al archivo en lineas si se visualice dentro del correo o algo pero es muy parecido a la funcion de arriba
    while True:
        ...
        if yesno.lower() == "y" or yesno.lower() == "yes":
            ...
            break
    #aqui se declara el asunto del correo
    subject = input(setprompt(["1"], "Email subject"))
    #dentro del try se pregunta si se va a enviar en formato de html o texto plano
    try:
        html_flag = input(
        ...
        # line
        # Aqui se agrega el texto que irá en el cuerpo del correo el cual termina cuando escribimos "END"
        while body != 'exit':
            try:
...
def mail(to, subject, prioflag1, prioflag2, text):
    #En esta funcion se mandan todos los datos recabados anteriormente para mandar el correo
    msg = MIMEMultipart()
    #from_displayname y from addres vienen de arriba como variables
    msg['From'] = str(
